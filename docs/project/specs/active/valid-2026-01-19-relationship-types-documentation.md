...
This validation spec documents the comprehensive new section added to tbd-design.md
that clarifies the relationship type model, comparing tbd's approach to Beads and
documenting the rationale for design differences.
...
This was a documentation clarification task. The goal was to clearly document:
...
None - all documentation has been added. The key insight is documented:
tbd intentionally uses a different model where `parent_id` is organizational only
(non-blocking), while Beads' `parent-child` dependency type blocks the ready queue.
