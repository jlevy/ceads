...
- Apply `configureHelp()` on root program AND all commands added via `.addCommand()`
  (they do NOT inherit parent settings).
  Commands created inline with `.command()` inherit automatically.
